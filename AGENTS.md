...
У нас есть платформа (внешняя система), где разные продуктовые модули отправляют пользователям
уведомления по нескольким каналам: email, SMS, мессенджеры, пуши.
Данный сервис отвечает за то, какие типы уведомлений и по каким каналам можно
отправлять конкретному пользователю с учётом его выбора, дефолтных настроек и
глобальных политик.
Этот сервис служит единым источником правды для остальных
